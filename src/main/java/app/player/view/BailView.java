    ...
    public boolean showMenuBail(final Player player) {
        ...
        if (choice == JOptionPane.YES_OPTION) {
            if (logic.hasPayed(player)) {
                ...
                JOptionPane.showMessageDialog(null, "Not enough money for paying the bail. TRY YOUR LUCK!",
                        "STILL IN PRISON", JOptionPane.ERROR_MESSAGE);
                bailResult = false;
            }
        } else {
            JOptionPane.showMessageDialog(null, "You have decided not to pay the bail. TRY YOUR LUCK!",
                    "STILL IN PRISON", JOptionPane.ERROR_MESSAGE);
            bailResult = false;
        }
        return bailResult;
